...
def plot_chart(df_plot, x_col, y_col, title, kind="bar"):
    ...
    if kind == "bar":
        bars = ax.bar(x_vals, df_plot[y_col], color="#4CAF50", alpha=0.9)
        for bar in bars:
            h = bar.get_height()
        ...
        ax.plot(x_vals, df_plot[y_col], marker="o", color="#2196F3", linewidth=2)
...
# --- Helper: build summary text for a grouped DF (value column) ---
...
if user_input:
    ...
    if re.search(r"\b(total\s+revenue|what\s+is\s+the\s+total\s+revenue|total\s+revenue\s+this\s+month|total\s+revenue\s+in\s+\d{4}|total\s+revenue\s+in\b)", q) \
       or ( "revenue" in q and ("total" in q or "this month" in q or re.search(r"\b\d{4}\b", q)) and not re.search(r"\b(hostel|mess|canteen|contractor|party)\b", q) ):
        ...
        # handle explicit year like "in 2024"
            ...
            if "date" in df.columns:
                ...
                with st.chat_message("model"):
                    ...
                    # also show month breakdown chart if user asked "by month" for that year
                    if "month" in q or "by month" in q:
            ...
            # generic total revenue across dataset
    ...
    # ---------- 4) Other queries: fallback to AI or preview ----------
        ...
        # If model available, ask for a natural language answer; otherwise show a small preview
        if model:
